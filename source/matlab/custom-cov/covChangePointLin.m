function K = covChangePointLin(cov, hyp, x, z, i)

% 1d change point kernel - piecewise linear transition
%
% k(x^p,x^q) = xxx
%
% hyp = [ location
...
ax = ((x > (location - 0.5*width)) & (x < location + 0.5*width)) .* ...
     (x - (location - 0.5*width)) ./ width ...
     + ...
     (x >= location + 0.5*width);
if ~dg
    ax = repmat(ax, 1, length(z));
end
if ~dg
    az = ((z > (location - 0.5*width)) & (z < location + 0.5*width)) .* ...
         (z - (location - 0.5*width)) ./ width ...
         + ...
         (z >= location + 0.5*width);
...
if nargin<5                                                        % covariances
  ...
  if i==1
    ...
    dx = -((x > (location - 0.5*width)) & (x < location + 0.5*width)) ./ width;
    dz = -((z > (location - 0.5*width)) & (z < location + 0.5*width)) ./ width;
...
%     dz(isnan(dz)) = 0; % infty * 0 = 0 for this function
%     dz(isinf(dz)) = 0; % infty * eps = 0 for this function
    dx = ((x > (location - 0.5*width)) & (x < location + 0.5*width)) .* (x - location) ./ width;
    dz = ((z > (location - 0.5*width)) & (z < location + 0.5*width)) .* (z - location) ./ width;
    dx = repmat(dx, 1, length(z));
    dz = repmat(dz', length(x), 1);
    dx = -dx; % Switching the order of base kernels to match intuition 
    dz = -dz;
