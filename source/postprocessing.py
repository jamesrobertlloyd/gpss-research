...
def collate_decompositions_cumulative(top_folder, tex):
    '''Produces a LaTeX document with all decompositions displayed'''
    latex_header = '''
\documentclass[twoside]{article}
\usepackage{algorithm}
...
\\newcommand{\mbm}{\hspace{-0.3cm}}
\\begin{tabular}{cc}
\mbm \includegraphics[width=\wmgd,height=\hmgd]{\mdrd/%(folder)s_all} & Cumulative fits\\\\ = \\\\
'''
...
'''
    ...
    decomp_body = '''
\mbm \includegraphics[width=\wmgd,height=\hmgd]{\mdrd/%(folder)s_%(number)d} & \includegraphics[width=\wmgd,height=\hmgd]{\mdrd/%(folder)s_%(number)d_cum} \\\\ + \\\\
'''
