function K = covChangeWindowMultiD(dim, cov, hyp, x, z, i)

% Multi d change window kernel using tanh
%
% k(x^p,x^q) = xxx
%
% hyp = [ location
...
if nargin<6                                                        % covariances
  ...
  for ii = 1:length(cov)                              % iteration over functions
    ...
    if ii == 2
        ...
        K = K + (1-ax) .* feval(f{:}, hyp([false false false (v==ii)]), x, z) .* (1-az);
    end
  end
else                                                               % derivatives
  if i==1
    ...
    for ii = 1:length(cov)                              % iteration over functions
        ...
        if ii == 2
            ...
            K = K + -dx .* feval(f{:}, hyp([false false false (v==ii)]), x, z) .* (1-az);
            K = K + (1-ax) .* feval(f{:}, hyp([false false false (v==ii)]), x, z) .* (-dz);
        end
    end
  elseif i==2
