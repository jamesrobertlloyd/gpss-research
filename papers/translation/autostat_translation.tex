...
We extend a recently introduced nonparametric model construction procedure to produce automatic natural-language description of data.  Our extended procecure takes a time-series as input, runs an open-ended model search, then produces a detailed report with figures and english text which decompose and describe the various types of structure discovered.  We discuss how the class of Gaussian process models being search over naturally decompose into interpretable structure, and introduce several extensions to this model class.  This model search also provides state-of-the-art predictive capability.
\end{abstract} 
...
Simple parametric regression models such as linear regression are usually easy to interpret, requiring only a table of regression coefficients and a few other parameters to describe the model\fTBD{RG:Arguably uninterpretable since coefficients can depend on what other features used. JL: Is this more a problem with the model itself?}.
...
In this paper, we extend that work by demonstrating that the compositional structure of the models being searched over allows for the automatic natural-language description of patterns within a data set.  We demonstrate a procedure which, given a dataset, produces a detailed report. % \ie the interpretable models are automatically interpreted!
%
...
The Gaussian process structure search (GPSS)\fTBD{RG: More specific name?} procedure \citep{DuvLloGroetal13} searches over sums and products of a set of simple base kernels to produce an appropriate model for a given data set.
...
For example, when applied to airline passenger data (section~\ref{sec:airline}), the resulting kernel contatined a component of the form $\kSE \times \kPer \times \kLin$, which could be described as ``locally periodic, with linearly growing amplitude''. \fTBD{Unfortunately, we never described it that way in the paper}
...
\fTBD{RG: If put after language generation can point to how generating language helped improve the space of kernels}
\fTBD{RG: Can be shortened - not all details interesting JL: Agreed}
...
To capture this type of phenomena, changepoints were introduced to the model language. \citep{garnett2010sequential, FoxDunson:NIPS2012}.
...
We introduce the white noise kernel\fTBD{RG: Change name to identity? Does not always produce white noise}, $\kWN$, as a base kernel.
...
%Changepoints, linear kernels and stationary kernels within a product can be described separately.
...
\TBD{To be improved and then worked into next section potentially}
...
\TBD{Please beautify and integrate this material into the previous section as you see fit. Potentially reordering with description section?}
...
\label{sec:examples}
\fTBD{RG: Add worked example?}
\fTBD{JTB: Remember that we have automated a few pages of GPML discussing Mauna Loa - especially since we automatically generate reports!}
...
The automatic natural-language descriptions objectively\fTBD{RG: Whether or not they are good is subjective} demonstrate that the models produced are interpretable.
...
\vspace{-0.5\baselineskip}
\fTBD{RG: Add CW to notation section?}
\fTBD{RG: Why not add constant to every kernel? JL: This would result in anti-correlation in the posterior - in general it is better to remove constants when possible}
\begin{itemize}
  \itemsep0em
  \item $\kC$
  \item $\kCW(\kC)$
...
\label{sec:numerical}
\fTBD{RG: What is the motivation of this section - seems orthogonal JL: Added to satisfy reviewers who need numerical evaluations - but this depends on the conference it is aimed at I suppose}
...
\TBD{
If we choose to present two versions of the algorithm (see figure \ref{fig:box_extrap_dist}) then we need to discuss how it works.
}
