...
\begin{frame}{How to do linear regression}
  \begin{block}{Example of linear regression data}  
  \end{block}
  \begin{block}{}
    \begin{itemize}
      \item Statistical linear regression starts by assuming a model of the form $y_i = m x_i + c$ where $x$ and $y$ are inputs and outputs respectively
      \vspace{\baselineskip}
      \item Common approach: Estimate $m$ and $c$ by least squares (show picture)
      \vspace{\baselineskip}
      \item Bayesian approach: Specify prior beliefs (show picture) about $m$ and $c$ using probability distributions and then follow axioms of probability to update beliefs rationally after observing data (show picture)
    \end{itemize}
  \end{block}
\end{frame}
