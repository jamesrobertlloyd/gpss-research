...
The parameters and design choices of these refinements have been chosen by our best judgement, but learning these parameters objectively from expert statisticians would be a profitable area of future study.
...
We now discuss how these differences improve the interpretability of the models produced by \procedurename{}.

\paragraph{Removal of rational quadratic kernel}

The rational quadratic kernel \citep[e.g.][]{rasmussen38gaussian} can be expressed as a mixture of $\kSE$ kernels.
...
Second, the constant functions can result in anti-correlation between components in the posterior, resulting in inflated credible intervals for each component (see figure~\ref{fig:constant}).
