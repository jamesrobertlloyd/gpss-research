...
We have begun to experiment with using posterior predictive checks to assess the models produced by GPSS following the techniques of \cite{Gelman1996} (see reports reports in the supplementary material).
...
We include learned equations for these two data sets produced using Eureqa \citep{Eureqa} using the default mean absolute error performance metrix\footnotemark.
...
We use the default mean absolute error criterion when using Eureqa\footnotemark.
The other four algorithms can be expressed as restrictions of our modelling language (see table~\ref{table:motifs}) so we perform inference using the same search methodology and selection criterion (Bayesian information criterion) with appropriate restrictions to the grammar.
