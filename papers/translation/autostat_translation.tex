...
The $\boldsymbol\sigma_m$ in equation~\eqref{eq:sop} represent products of sigmoids which also have the form $a(x)a(x')$ and are therefore described as multiplication of functions.
For simplicity we treat sigmoids as step functions\footnotemark.
These step functions are represented as intervals (\ie where their value is 1), and their product is equivalent to taking the intersection of intervals.
Computing the intersection is trivial and descriptions are templated as \eg `this function applies from [start] until [end]'.
