...
For example, when applied to airline passenger data (see section \ref{sec:airline}), the resulting kernel contained a component of the form $\kSE \times \kPer \times \kLin$, which captured a feature of the data which can be described as \genText{``approximately periodic, with linearly growing amplitude''}.
...
\label{eq:sop}
\end{align}
where $\kLin^m$ denotes a product of $m$ $\kLin$ kernels, possibly with different parameters.\fTBD{DD: We're not sure about notation here}
...
Under an $\kSE$ kernel, distant points are uncorrelated. Multiplying by $\kSE$ removes any long range correlation, i.e.~it modifies a global covariance structure to hold only locally.
...
Observe that if $f(x)$ is a draw from a \gp{} with kernel $k$, and $a(x)$ is a deterministic function, then 
...
The changepoint kernel $\boldsymbol\sigma$ in equation~\eqref{eq:sop} is simply a product of sigmoids, $\sigma(x)\sigma(x')$.  Its modifier can therefore be described as multiplication by a sigmoid, meaning that the function being modeled goes to zero in some region.
%For simplicity we describe these sigmoids as step functions\footnotemark.
Products of sigmoids can be seen as intervals (regions where their value is close to 1).
...
This work was partly inspired by previous successes at searching over open-ended model spaces: matrix decompositions \citep{grosse2012exploiting}, graph structures \citep{kemp2008discovery}, and 3D scenes \cite{VikashScene13}.\fTBD{RBG: I'm not familiar with the Vikash reference; does it fit in this context?}
