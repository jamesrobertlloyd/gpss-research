...
\fTBD{JBT: Stress the \emph{automation}}
...
However, many datasets are too complex to be accurately described by simple parametric models such as linear regression.
Non-parametric regression models are flexible enough to model complex distributions over observables, but their construction can require considerable expertise.
...
Unfortunately, the outputs of their procedure still required analysis and interpretation by human experts.
...
We call the new procedure Automatic Bayesian Covariance Discovery (\procedurename)\fTBD{Careful with always adding a space after the procedurename name - do we need a gp\{\} construction?}.
\fTBD{DD:  or maybe GP structure discovery - GPSD?}
\fTBD{JL: Is this the correct point at which to introduce the name - we have only talked about the language at this stage - maybe it should be introduced at the end of the previous section - \eg we attempt to address all of the above with our new algorithm \procedurename}
...
\begin{itemize}
\item We added the changepoint and window operators, which capture phenomena which occur only during certain time periods.
...
\item We replaced the periodic kernel with a more flexible one which includes the cosine kernel of \citet{WilAda13} as a limiting case. \citet{WilAda13} demonstrated that any stationary kernel can be approximated as a sum of kernels of this form.
\end{itemize}
...
%, it doesn't seem like such an expensive process.
