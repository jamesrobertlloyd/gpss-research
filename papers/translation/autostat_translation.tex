...
It is not clear\fTBD{JL: Weak} what obstacles there are to building a system capable of explaining novel statistical structures.  However, in this work, we attempted to build such an automatic summarization procedure, and found that the properties of the model class being explored (Gaussian processes) allowed the models discovered to be explained in a modular way.  This modularity means that a large class of statistical structures can explained, without resorting to writing a special case for every possible structure in our open-ended set of models.
...
To complement our demonstration of the interpretability of \procedurename we conducted numerical experiments testing the ability of various model building algorithms to interpolate and extrapolate time-series.
...
In addition to the three time series analysed in \cite{DuvLloGroetal13} we evaluate the performance of the algorithms listed below on 10 additional time series from the time series data library \citep{TSDL}; plots of the data can be found at the beginning of the reports in the supplementary material\fTBD{Assuming all the reports fit within size constraints}.
...
Typical machine learning\fTBD{JL: Suitable language for AAAI?} research focuses on numerical performance rather than interpretability, likely due to the difficulty of assessing the interpretability of a method in an objective manner.
...
Requiring that statistical models can be automatically described constrains the models that can be used.
