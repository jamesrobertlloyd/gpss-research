...
\NA{
This allows for the procedure to fit and describe many well established modelling motifs including, linear regression (cite), fourier analysis (cite), changepoints (cite), heteroscedasiticty (cite), trend periodic short term (cite) and nonparametirc additve / MKL (cite).
}
...
\NA{
linear regression (cite), fourier analysis (cite), changepoints (cite), heteroscedasiticty (cite), trend periodic short term (cite) and nonparametirc additve (cite) and andrews work (cite) - do we also want multidimensional things?
}
...
The vast majority of machine learning focuses on numerical performance, with claims of interpretability generated post-hoc.
...
We compare with the learned equations for these two data sets for completeness; however, it is very unlikely that these data sets can be explained parsimoniously with a parametric function.

The learned equation for the solar data is
\begin{equation}
Irradiance = 1361 + 0.6sin(3.244 + 0.008617t)sin(0.4533 + 6.111e-5t^2 - 0.2254t)
\end{equation}
which captures the constant offset and models the long term trends with a product of sinusoids but fails to capture the solar cycle or the Maunder-minimum.

The learned equation for the airline data is
\begin{equation}
Passengers = 32.28t + 85.17cos(1.037 - 56.6t)logistic(0.3148t - 615.6) - 6.282e4
\end{equation}
...
To complement our demonstration of the interpretability of our method, we conduct numerical experiments testing the ability of the models learnt by the structure search algorithm to interpolate and extrapolate time-series.
The main conclusion of these experiments is that the structure search algorithm consistently performs well, whilst more restricted model classes can occasionally perform poorly on data sets that violate their more rigid assumptions.
They also hint at improvements to the structure search language which we leave for future work.

\paragraph{Data sets}

In addition to the three time series analysed in \cite{DuvLloGroetal13} we evaluate the performance of various algorithms on 10 additional time series chosen from (cite time series data library); plots are given in the appendix.
...
These four algorithms can be expressed as restrictions of our modelling language see table (add reference).
...
Further inspection of the extrapolation has revealed that while TCI cannot model stationarity, its extrapolations of approximately periodic components can be quite effective.
...
This suggests that models composed of kernels of the form $(\kSE + \kC) \times \kPer$ will be effective for extrapolating approximately periodic components; we leave this for future research.
...
\paragraph{Disconnected thoughts}

\NA{
This is not really a machine learning paper, rather we are using statistical tools developed by machine learning folks to automate the process of statistical analysis.
The predictive performance of these models is in the spirit of machine learning.
}
