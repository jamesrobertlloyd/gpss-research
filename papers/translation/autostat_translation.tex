...
Each kernel in a product modifies the resulting model in a consistent way, resulting in a highly modular text generation algorithm.\fTBD{JL $\to$ DD: I really don't find this work as surprising as you! Can we stop using words like `surpsingly' and `luckily'? :)}
...
As an example, a kernel of the form $\kSE \times \kPer \times  \kLin \times \boldsymbol{\sigma}$ can be described as an\fTBD{JL: Changed `local' to `approximate' based on poll in lab}
\begin{align*}
...
\footnotetext{In fact, all kernels could be described as adjectives by always multiplying the kernel product by the constant $\kC$ kernel to provide a noun.}
...
%
The second component could be described as `A smooth function with a lengthscale of [lengthscale]', corresponding to the $\kSE$, 'which applies until [changepoint]', which corresponds to the $\boldsymbol{\sigma}$.
%
