...
%Just as a human statistician cannot try every possible model, 
...
Because any search in an open-ended space must start with relatively simple models before moving on to more complex ones, any model search in an open-ended space will likely resemble a model-building procedure.% based on the results of simpler model fits.
%The search algorithm can have a large effect on whether the overall procedure is consistent.
...
\TBD{\citet{gelman2012philosophy} reviews the literature on these questions. 
