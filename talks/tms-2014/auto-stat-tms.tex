...
\begin{frame}{How to do linear regression} 
  ...
  \end{center}
  \begin{itemize}
    \item Statistical linear regression starts by assuming a model of the form $y_i = m x_i + c + \varepsilon_i$ where $x$ and $y$ are inputs and outputs respectively and $\varepsilon$ are errors or noise
  \end{itemize}
\end{frame}
...
\begin{frame}{How to do linear regression} 
  ...
  \end{center}
  \begin{itemize}
    \item Common approach: Estimate $m$ and $c$ by least squares
  \end{itemize}
\end{frame}
...
\begin{frame}{How to do linear regression} 
  ...
  \end{center}
  \begin{itemize}
    \item Bayesian approach: Specify prior beliefs about $m$ and $c$ using probability distributions
  \end{itemize}
\end{frame}

\begin{frame}{How to do linear regression} 
  \begin{center}
    \includegraphics<1>[width=0.8\textwidth]{figures/lin_reg/bayes_5}
    \includegraphics<2>[width=0.8\textwidth]{figures/lin_reg/bayes_10}
    \includegraphics<3>[width=0.8\textwidth]{figures/lin_reg/bayes_15}
  \end{center}
  \begin{itemize}
    \item Bayesian approach: Specify prior beliefs about $m$ and $c$ using probability distributions and then follow axioms of probability to update beliefs rationally after observing data
  \end{itemize}
\end{frame}

\begin{frame}{Bayesian statistics}
  \begin{block}{Sum and product rules of probability}
    \begin{align*}
...
\begin{frame}{Bayesian statistics}
  ...
  \begin{block}{Bayes rule}
    \vspace{-\baselineskip}
    \begin{align*}
      p(\theta\,|\,D,M) = \frac{p(D\,|\,\theta,M)\,p(\theta\,|\,m)}{\int p(D,\theta\,|\,M)\,\textrm{d}\theta}
    \end{align*}
    where $D$ represents data and $\theta$ are parameters of a model $M$
  \end{block}
\end{frame}
...
\begin{frame}{Bayesian statistics}
  ...
  \begin{block}{Bayes rule}
    \vspace{-\baselineskip}
    \begin{align*}
      \textrm{Posterior beliefs} = \frac{\textrm{Likelihood} \times \textrm{Prior beliefs}}{\textrm{Marginal likelihood}}
    \end{align*}
    \ie a formula to convert prior beliefs into posterior beliefs
  \end{block}
\end{frame}
...
\begin{frame}{Bayesian linear regression}
  \begin{itemize}
    \item Statistical linear regression starts by assuming a model of the form $y_i = m x_i + c + \varepsilon_i$ where $x$ and $y$ are inputs and outputs respectively and $\varepsilon$ are errors or noise
    \vspace{\baselineskip}
    \item Represent prior beliefs about $m$ and $c$ using probability distributions
    \begin{itemize}
      \item \eg $m \sim \textrm{Normal}(\textrm{mean} = 0,\textrm{variance} = 1)$
      \item We will asume $c = 0$ for simplicity
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Bayesian linear regression}
  \begin{itemize}
    ...
    \end{itemize}
    \vspace{\baselineskip}
    \item Represent prior beliefs about $\varepsilon$ using a probability distribution
    \begin{itemize}
...
\begin{frame}{Bayesian linear regression}
  ...
  \pause
  \begin{align*}
    p(m\,|\,y,x) = \frac{\big(\prod_i\frac{1}{2\pi\sigma_\varepsilon}e^{-(y_i-mx_i)^2/(2\sigma_\varepsilon^2)}\big)\frac{1}{2\pi}e^{-m^2/2}}{\int p(y,m,|\,x)\,\textrm{d}m}
  \end{align*}
  \vspace{-\baselineskip}
  \pause
  \begin{align*}
...
\begin{frame}{Bayesian linear regression}
  \begin{itemize}
    ...
    \vspace{\baselineskip}
    \pause
    \item This implictly defined a prior on $\{y_i \,:\, i=1,\ldots,n\}$
    \pause
    \begin{itemize}
      \item $y_i = mx_i + \varepsilon_i$
      \pause
      \item $y_i \sim \mathcal{N}(0, x_i^2 + \sigma_\varepsilon^2)$
      \pause
...
\begin{frame}{Gaussian processes}
  ...
  We define the kernel function $k(x_i, x_j) = x_i x_j + \delta_{ij}\sigma_\varepsilon^2$
  
  \vspace{\baselineskip}
  \pause
  
  The kernel function is consistent for any choice of $x_i$
...
\begin{frame}{Definition: Gaussian processes}
  \begin{itemize}
