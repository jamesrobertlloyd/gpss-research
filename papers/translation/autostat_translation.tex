...
%\\
  This paper presents the beginnings of an automatic statistician,
  focusing on regression problems. Our system explores an open-ended
  space of possible statistical models to discover a good explanation
  of the data, and then produces a detailed report with figures and
  natural-language text.

% Unlike classical equation learning systems, 

Our approach treats unknown functions nonparametrically using Gaussian
processes, which has two important consequences.

First, Gaussian processes model functions in terms of high-level
properties (e.g.\ smoothness, trends, periodicity,
changepoints). This, taken together with the compositional structure of
our language of models, allows us to automatically describe functions 
through a decomposition into additive parts.
...
One aim of statistics is to produce accurate and concise descriptions
of data which yield insight into underlying sources of variability.
While simple models (such as linear regression) are largely automated
by standard software packages, there has been little work on the
automatic construction of flexible but interpretable models. 
Automating the process of statistical modelling would have a tremedous
impact fields that currently rely on  expert statisticians, machine
learning researchers, and data scientists.
What are the ingredients required for an Artificial Intelligence
system to be able to perform statistical modelling automatically? 
In this paper we conjecture that the following ingredients may be
useful for building an AI system for statistics, and we develop a
working system which incorporates them: 
\begin{itemize}
...
\item {\bf A principled method for evaluating models} during the
  search in terms of their complexity and their degree of fit to the
  data
...
space is searched greedily, using marginal likelihood computations and
...
%Finally, we demonstrate that the extended model construction
%procedure is comparable or superior to other model construction
%techniques at extrapolation tasks.
...
over the 13 time series. In the remainder of the paper we describe the
components of ABCD in detail. 

% We believe our system is a step towards automating much of the
% process of model construction, fitting, and analysis, and should
% allow a broader range of non-experts to fit and interpret
...
represent both simple parametric forms of $f$ such as linear,
polynomial, etc, and also some complex nonparametric functions
specified in terms of properties such as smoothness, periodicity,
etc. Fortunately, Gaussian processes (\gp{}s) provide a very general
...
% what is a GP /  kernel
Gaussian processes are distributions over functions $f$ such that any
...
mean function, $\mu(x)=E(f(x))$ and kernel (or covariance) function
$k(x,x') = Cov(f(x),f(x'))$. It common practice to assume zero mean,
...
The elements of this language of kernels are a set of  base
...
Our base kernels are <base kernels> which on their own encode for
following properties:  <properties>.  The composition operators are
<blah>, where ``+'' means $k(x,x') = k_1(x,x') + k_2(x,x')$ for all
$x$,$x'$. 
Changepoint means <blah>

Using these base kernels and composition rules, it is possible to
express a wide variety of common regression motifs (<table>).

% relation to previous paper
This kernel composition framework builds upon the work of Duvenaud et
al (ref). We have expanded the framework of Duvenaud et al in the
following ways. <add here> In particular, we have found that
incorporating changepoints into the language is essential for
realistic models of time series (e.g.\ Figure~1). Moreover, in order
to achieve the goal of producing an interpretable decomposition of the
function, we had to take care to ensure that our base kernels were
parameterised so as to be roughly orthogonal and amenable to automatic
description. For example, the periodic kernel in Duvenaud et al, has a
constant component that can be removed to improve interpretability,
and the RQ kernel used by Duvenaud can be expressed as a sum of SE
kernels.
...
Because of their flexibility, \gp{}s are commonly used for
nonparametric regression.

\fTBD{JL: I think these first two sentences can be improved - maybe
  start of with a more abstract definition and then explain that this
  allows them to specify flexible distributions over functions - hence
  the use for nonparametric regression}

A \gp{} regression model is typically specified by a kernel function ${\kernel : \mathcal{X}^2 \to \mathbb{R}}$, which defines the covariance between the function values $(\outputVar, \outputVar')$ evaluated at two inputs $(\inputVar,\inputVar')$, \ie ${\textrm{Cov}(\outputVar, \outputVar') = \kernel(\inputVar,\inputVar')}$.
The kernel determines which structures are likely under the \gp{} prior, which determines how the model generalizes or extrapolates.
...
In this work, we use a similar space of models, but with improvements which are discussed in section~\ref{sec:related-work}\fTBD{JL: Can we quickly highlight the additions and then refer to a resurrected regression motifs table to show the extension visually}. 

\fTBD{DD: Do we want to distinguish between kernels and kernel families?}\fTBD{JL: I'm ok with overloading the word `kernel'}The buildling blocks of our model space are a handful of ``base'' kernel families with simple parametric forms.
Each kernel family has one or more parameters; when the parameters are specified, it defines a kernel function.
Each kernel family favors particular kinds of structure; e.g., a \gp{} with a squared-exponential kernel (denoted $\kSE$) favors smooth functions, while a \gp{} with a periodic kernel ($\kPer$) favors periodic functions.
The full set of base kernel families is given in Table~{\bf TODO}\fTBD{JL: Lets do this EARLY given that space it at a premium}.

The set of kernel functions is closed under addition and multiplication, hence any algebraic expression consisting of $+$, $\times$, and base kernel families defines a kernel family. 
Such composite kernels can describe richer structures not representable by the base kernels.
For example, the kernel $\kSE \times \kPer$ puts mass on functions which are locally\fTBD{JL: local / approximate - lets be consistent} periodic, while a \gp{} with kernel $\kSE + \kLin$ corresponds to a prior on smooth functions with a linear trend.
The procedure of \citet{DuvLloGroetal13}\fTBD{JL: Can this paragraph not mention the search procedure? And then be shorter?} automatically searched over the space of composite kernels (represented as algebraic expressions), where the search operators consisted of modifying sub-expressions. 
By combining kernels, the search procedure was able to construct novel nonparametric regression models.
For example, when applied to airline passenger data (see section \ref{sec:airline}), the resulting kernel contained a component of the form $\kSE \times \kPer \times \kLin$, which captured a feature of the data which can be described as \genText{``approximately periodic, with linearly growing amplitude''}\fTBD{JL: Why this formatting!?}.
...
%
We call the entire model-search and description procedure automatic Bayesian covariance decomposition \fTBD{DD: D for decomposition or description?  We can also add E for explanation or extrapolation.} (\procedurename{}).
...
Search is greedy breadth first search using the same search operators
as Duvenaud plus extra ones to incorporate changepoints and others
(see Supplementary Material). 

After each model is proposed its kernel hyperparameters are optimised
(cite RasWil). 

We evaluate the fit of a model to the data by using the following
score function 
\[
score_D(m) = \log P(D|m) - \frac{p}{2} \log n
\]
where $p$ is the number of optimised hyperparameters in the
kernel, and $\log P(D|m)$ is the \gp{} marginal likelihood which can
be evaluated analytically, and BIC (ref Schwartz 1978) is a complexity
penalty. This combined score trades off model fit and complexity and
implements what is known as ``Bayesian Occam's Razor'' (cite MacKay;
Carl+ZG).
...
\fTBD{DD: This section should emphasize the main ideas that make automatic description possible}
...
In this section, we outline the procedure by which we\fTBD{JL: We use `we' too often - here we are describing how the automatic producedure generates text - even if we told it how to do it} generate natural-language descriptions of any composition of base kernels.
%
First\fTBD{JL: There is no `second' - kill}, we note\fTBD{JL: Unnecessary words - see \eg \url{http://andrewgelman.com/2014/01/14/advice-writing-research-articles/}} that because\fTBD{JL: because} products distribute over sums, any kernel in our space can be rewritten as a sum of products of base kernels.
For example, $\kSE \times (\kC + \kLin) = \kSE \times \kC + \kSE \times \kLin$.
We observe that addition and multiplication appear to play complementary roles\fTBD{JL: Let's say something exact}: a sum of multiple kernels corresponds to a superposition of different functions\fTBD{JL: Citation or proof needed}.
Therefore, we\fTBD{JL: Too much `we'} report to the user a list of components, given as noun phrases, one for each term in the sum.
By\fTBD{JL: Surely `in'} contrast, multiplying one kernel by another often corresponds to modifying the original kernel in a particular way\fTBD{JL: This is a very fuzzy sentence}.
Different terms in the product correspond to different parts of the noun phrase.
...
The individual product terms can be further simplified.
The product of two $\kSE$ kernels is a $\kSE$ kernel with the same form. Multiplying $\kWN$ by any stationary kernel ($\kC$, $\kWN$, $\kSE$, or $\kPer$) gives another $\kWN$ kernel. Multiplying any kernel by $\kC$ only changes the parameters of that kernel. By applying these rules, the kernel family can be rewritten such that all of the terms have the following form:
%\begin{align}
...
\label{eq:sop}
\end{align}
where $\kLin^m$ denotes a product of $m$ $\kLin$ kernels, possibly with different parameters.\fTBD{DD: We're not sure about notation here}\fTBD{JL: The changepoint notation is not explained due to deleted content. We should attack this EARLY since this could require a lot of reorganisation}
...
This ordering is based on our intuitive judgment of which component contributes the most to the overall structure of the kernel; e.g., $\kSE \times \kPer$ is more similar to $\kPer$ than to $\kSE$, and $\kLin \times \kSE$ is more similar to $\kSE$ than to $\kLin$.\fTBD{JL: Rather I would say that this ordering is the one that gives the most intuitive / interpretable phrases and leave it at that}
...
In particular, we use the following head nouns: \fTBD{ is head noun the right terminology, since these are noun phrases?}

\begin{tabular}{ll}
$\kPer$ & \genText{periodic function} \\
$\kSE$ & \genText{smooth function} \\
$\kWN$ & \genText{uncorrelated noise} \\
$\kC$ & \genText{constant} \\
$\kLin$ & \genText{linear function} \\
$\kLin^n$ & \genText{polynomial of degree $n$} ($n \geq 2$) \\
\end{tabular}
...
The remaining terms in the product each contribute modifiers to the noun phrase. Because $\kPer$ kernels are always chosen as the head noun and $\kC$ terms are absorbed into other kernels, the only terms which can contribute modifiers are $\kSE$, $\kLin$, and changepoints\fTBD{JL: Make sure we have mentioned changepoints already}.\fTBD{JL: It is possible to describe products of periodics - but lets be honest that the description is about covariance only}\footnote{While products of multiple $\kPer$ kernels are technically possible, we found this to be rare, so we omit this case for brevity.} The modifiers are as follows:
...
Under an $\kSE$ kernel, distant points are uncorrelated.
Multiplying by $\kSE$ removes any long range correlation\fTBD{JL: Citation or proof needed -  we are we avoiding mathematical content?}, i.e.~it modifies a global covariance structure to hold only locally.
In our language, $\kSE$ corresponds to a modifier only when $\kPer$ is present; it contributes the modifier \genText{`approximately'}.
...
%\fTBD{DD: This paragraph is baroque and confusing.}
...
Observe that\fTBD{JL: unnecessary words} if $f(x)$ is a draw\fTBD{JL: This is the first time we say `draw' - what was wrong with the $\dist$ notation?} from a \gp{} with kernel $k$, and $a(x)$ is a deterministic function, then 
$a(x)f(x) \dist \gp{}\left(0, a(x) k(x, x') a(x') \right)$.
Since $\kLin$ has this form, where $a(x)$ is linear, multiplying by $\kLin$ is equivalent to multiplying the draw $f(x)$ by a linear function.
This is described with the modifier \genText{`with lineary growing standard deviation'}.
Similarly, multiplication by $\kLin^n$ corresponds to the modifier \genText{`with polynomially varying standard deviation}.\footnote{For stylistic reasons, we use \genText{'linearly growing amplitude'} in place of \genText{'linearly growing standard deviation'} when $\kPer$ is the head noun.}

%We include a special case when $\kPer$ is the head noun \eg \genText{`linearly growing amplitude'} to produce more natural sounding text.
...
The changepoint kernel\fTBD{JL: Operator / kernel - we need to fix how we introduce and talk about changepoints} $\boldsymbol\sigma$ in equation~\eqref{eq:sop} is simply a product of sigmoids, $\sigma(x)\sigma(x')$.
Its modifier can therefore be described as multiplication by a sigmoid, meaning that the function being modeled goes to zero in some region.
%For simplicity we describe these sigmoids as step functions\footnotemark.
Products of sigmoids can be seen as\fTBD{JL: Or rather they can be approximated as} intervals (regions where their value is close to 1).
%, and their product is equivalent to taking the intersection of intervals.
This corresponds to the post-modifier \genText{`this function applies from [start] until [end]'}.
...
Additional modifiers are added depending on the parameters of the head noun.  \fTBD{add some examples}
\fTBD{JL: The above description is true for the short descriptions - extra things are added to the long descriptions - but let's say that we did this since we are still experimenting or something like that.}
\fTBD{JL: But also, see Roger's comment at the end of this section}
...
To descibe the first component, the head noun template for $\kWN$, \genText{`uncorrelated noise'}, is concatenated with a modifier template (chosen based on parameter values) for $\kLin$, \genText{`with linearly increasing standard deviation'}.
%
The second component is described as \genText{`A smooth function with a lengthscale of [lengthscale] [units]'}, corresponding to the $\kSE$, \genText{'which applies until [changepoint]'}, which corresponds to the $\boldsymbol{\sigma}$.
%
Finally, the third component is described as \genText{`An approximately periodic function with a period of [period] [units] which applies from [changepoint]'}.
...
\fTBD{RBG: We don't necessarily need to mention this, as long as the generation isn't conceptually different.}
