...
Non-parametric regression models such as Gaussian processes are flexible enough to model complex nonlinear phenomena.
Unfortunately, their design and interpretation can require considerable human time and expertise, and the results are inevitably filtered through the preconceived notions of the researchers\fTBD{JL: I like this paragraph, but maybe this last clause is a little strong}.
...
\begin{itemize}
\item {\bf An open-ended language of models} expressive enough to capture many of the modeling assumptions and model construction techniques applied by human statisticians
\item {\bf A search procedure} which proposes promising models to fit based on the results of fitting previous models
\item {\bf Methods for evaluating a model} in terms of the complexity of the model and the degree of fit to the data
\item {\bf Automatic generation of reports} which explain and visualize different factors underlying the data, make the chosen modeling assumptions explicit, and quantify how each component improves the predictive power of the model
\end{itemize}
...
In this manuscript, we present a procedure\fTBD{JL: introduce the name here?} which uses the compositional structure of Gaussian process kernels to automatically produce natural-language descriptions of patterns in a data set.
...
%
We show examples of automatically generated reports which highlight interpretable features of a variety of data sets.
...
The supplementary material to this paper includes 13 complete reports generated by our method, all using the same code with no hand-tuned parameters.
...
Gaussian processes (\gp{}) \citep{rasmussen38gaussian} are distributions over functions such that any finite subset of points have a joint Gaussian distribution.
Because of their flexibility, \gp{}s are commonly used for nonparametric regression.\fTBD{JL: I think these first two sentences can be improved - maybe start of with a more abstract definition and then explain that this allows them to specify flexible distributions over functions - hence the use for nonparametric regression}
