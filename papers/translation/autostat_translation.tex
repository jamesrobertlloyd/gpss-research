...
There are also some special cases where the modularly produced wording is cumbersome.
...
\paragraph{Products of kernels distribute over addition}
...
Multiplying any kernel by the constant kernel, $\kC$ only changes its parameters.
...
After distributing products and simplifying all kernel expressions will be a sum of kernels of one of the following forms:\fTBD{JL: Not enitrely clear / canonical - 1 form - or many - or something else?}
\begin{align}
...
As a simple example, a kernel of the form $\kSE \times \kPer \times  \kLin \times  \sigma(x)$ could be described as:
\begin{align*}
