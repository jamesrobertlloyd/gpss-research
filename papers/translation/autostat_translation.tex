...
This allows for the procedure to fit and describe many well established modelling motifs including, linear regression, Fourier analysis, sparse spectrum \gp{}s\TBD{cite}, changepoints\TBD{cite}, heteroscedasiticty\TBD{cite}, trend-cyclical-irregular\TBD{cite} and nonparametric additive / MKL\TBD{cite}.
...
We have begun to experiment with using posterior predictive checks to assess the models produced by GPSS following the techniques of (cite Gelman, Meng and Stern) (see reports reports in the supplementary material).
...
In contrast, equation learning (cite) can also be used to automate the process of interpretation/.

We include learned equations for these two data sets produced using Eureqa (cite) using the default mean absolute error performance metrix\footnotemark.
...
In addition to the three time series analysed in \cite{DuvLloGroetal13} we evaluate the performance of GPSS and 5 other algorithms on 10 additional time series from (cite time series data library); plots can be found at the beginning of the reports in the supplementary material.

\paragraph{Algorithms}

We compare the structure search algorithm to equation learning (EL) \cite{Schmidt2009b} \fTBD{interpolation work in progress} and four other regression algorithms; additive nonparametric (cite) (SE), change point modelling (cite) (CP) / multi resolution Gaussian processes (cite), spectral kernels (SP) (cite) and trend-cyclical-irregular (TCI) (cite).\fTBD{I could add a single Sq-exp GP as a reference point?}

We use the default mean absolute error criterion when using Eureqa.
The other four algorithms can be expressed as restrictions of our modelling language (see table~\ref{table:motifs}) so we perform inference using the same search methodology and selection criterion (Bayesian information criterion) with appropriate restrictions to the grammar\footnotemark.
...
We restricted to regression algorithms for comparability; this excludes models which regress on previous values of times series (\eg cite ARIMA or similar).
